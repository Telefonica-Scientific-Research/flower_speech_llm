#!/bin/bash
#SBATCH --job-name=fl_B2_speech_llm
#SBATCH --output=./slurm_logs/fl_B2_speech_llm_%j.out
#SBATCH --error=./slurm_logs/fl_B2_speech_llm_%j.err
...
echo "FL Experiment B2: Non-IID speaker + FedProx"
echo "=========================================="
...
import yaml
cfg = yaml.safe_load(open('configs/fl_B2_speaker_fedprox.yaml'))
print(' '.join(f'{k}=\\\"{v}\\\"' if isinstance(v, str) else f'{k}={v}' for k,v in cfg.items() if not k.startswith('_')))
\")\"

echo 'Experiment B2 completed successfully'
"
