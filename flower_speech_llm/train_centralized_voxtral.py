...
def _build_run_dir(args):
    ...
    Layout: <output-dir>/<model>_lr<r>a<alpha>_enc<0|1>_llm<0|1>_bs<B>x<A>_<YYYYMMDD_HHMMSS>/
    Copies the config YAML into the run directory for reproducibility.
    """
    model_short = args.voxtral_model_name.split("/")[-1].lower()
...
class LogProgressBar(TQDMProgressBar):
    ...
    def init_train_tqdm(self):
        ...
        bar.ncols = 120
        # Force newline so each update is a new line in the log file
        bar.ascii = True
        return bar
